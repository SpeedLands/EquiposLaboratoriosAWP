...
function get_equipos($pdo) {
    $busqueda = $_GET['q'] ?? '';
    if ($busqueda) {
        $stmt = $pdo->prepare("SELECT * FROM equipos_laboratorios WHERE nombre LIKE ? OR categoria LIKE ? OR ubicacion LIKE ? ORDER BY id DESC");
...
function add_equipo($pdo) {
    $data = json_decode(file_get_contents('php://input'), true);
    $sql = "INSERT INTO equipos_laboratorios (nombre, categoria, serial, fecha_adquisicion, ubicacion, cantidad, imagen) VALUES (?, ?, ?, ?, ?, ?, ?)";
    $stmt = $pdo->prepare($sql);
    $stmt->execute([
        $data['nombre'], $data['categoria'], $data['serial'],
        $data['fecha_adquisicion'], $data['ubicacion'], $data['cantidad'], $data['imagen'] ?? null
    ]);
...
function update_equipo($pdo) {
    ...
    $sql = "UPDATE equipos_laboratorios SET nombre = ?, categoria = ?, serial = ?, fecha_adquisicion = ?, ubicacion = ?, cantidad = ?, imagen = ? WHERE id = ?";
    $stmt = $pdo->prepare($sql);
    $stmt->execute([
        $data['nombre'], $data['categoria'], $data['serial'],
        $data['fecha_adquisicion'], $data['ubicacion'], $data['cantidad'], $data['imagen'] ?? null,
